...
if (canvas && !matchMedia('(max-width:760px), (pointer:coarse) and (max-width:1024px)').matches) {
  ...
  const content = {
    projects:{index:'01',label:'SELECTED PROJECTS',title:'Two projects, shown with evidence.',summary:'The portfolio focuses on TabletopForge and the NIST CFReDS Hacking Case, with supporting scanner and GRC records available for added breadth.',target:[-.43,2.7,-.82],position:[2.9,3.35,5.0],body:`<figure class="panel-proof"><img src="assets/tabletopforge-generator.png" width="1440" height="900" loading="lazy" decoding="async" alt="TabletopForge exercise generator showing organization, industry, scenario, and scope inputs"><figcaption>TabletopForge generator · captured from the working project</figcaption></figure><h3>Selected projects</h3><a class="panel-link" href="case-tabletopforge.html"><span><strong>TabletopForge</strong><br>AI-assisted incident-response tabletop exercises</span><span>Case study ↗</span></a><a class="panel-link" href="case-hacking-case.html"><span><strong>NIST CFReDS Hacking Case</strong><br>Disk, registry, and network forensics</span><span>Case study ↗</span></a><h3>Supporting work</h3><a class="panel-link" href="case-vulnerability-scanner.html"><span>Windows Vulnerability Scanner</span><span>Case file →</span></a><a class="panel-link" href="case-grc-risk-assessment.html"><span>GRC Risk Assessment</span><span>Case file →</span></a><a class="panel-link" href="writeup-grc-risk-lessons.html"><span>GRC Risk Lessons</span><span>Field note →</span></a>`},
  ...
  const showFallback=message=>{loading?.classList.add('is-done');document.body.classList.add('scene-fallback-active');const status=document.querySelector('[data-fallback-status]');if(status){status.hidden=false;status.textContent=message;}};
  ...
  try {
    ...
    document.querySelectorAll('[data-focus]').forEach(button=>button.addEventListener('click',()=>{focusObject(button.dataset.focus);const help=document.querySelector('#help-panel');help.hidden=true;document.querySelector('[data-help]').setAttribute('aria-expanded','false');}));
    ...
    clearTimeout(loadingFailsafe);document.body.classList.remove('scene-fallback-active');document.body.classList.add('scene-ready');const fallbackStatus=document.querySelector('[data-fallback-status]');if(fallbackStatus)fallbackStatus.hidden=true;
    ...
    clearTimeout(loadingFailsafe);console.error('The 3D workspace could not start.',error);showFallback('Interactive mode could not start on this device. Open the complete portfolio overview instead.');
  }
}
